...
func (c *WaterfallClient) smallestLayerSize(nRows int) int {
	...
	return pir.SecParam * pir.SecParam
}
...
func (c *WaterfallClient) LayersMaxSize(nRows int) []int {
	...
	// 	return []int{}
	// }
	if c.pirType != pir.Punc && c.pirType != pir.SinglePass {
		return []int{nRows}
	}
	maxSize := []int{nRows}
...
func (c *WaterfallClient) HintUpdateReq(numNewRows int, rowLen int) (*UpdatableHintReq, error) {
	...
	if layer.pirType == pir.SinglePass {
		return &UpdatableHintReq{
		FirstRow: layer.firstRow,
		NumRows:  layer.numRows,
		Req:      pir.NewSinglePassHintReq(c.randSource,0),
		}, nil
	}

	//if layer.pirType != pir.Punc {
	req, err := pir.NewHintReq(c.randSource, layer.pirType,0).Process(pir.StaticDB{NumRows: layer.numRows, RowLen: c.rowLen}) //how can we process with an empty db? this if statement not functional (legacy)
	if err != nil {
		return nil, err
	}
...
func (c *WaterfallClient) updateLayers(numNewRows int) int {
	...
	if i == len(c.layers)-1 && (c.pirType == pir.Punc || c.pirType == pir.SinglePass) {
...
func (req UpdatableQueryReq) Process(db pir.StaticDB) (interface{}, error) {
	...
	for l, q := range req.Reqs {
		...
		resps[l], err = q.Process(pir.StaticDB{req.FirstRow[l+1] - req.FirstRow[l], db.RowLen,0, layerFlatDb})
		if err != nil {
			return nil, err
		}
	}
